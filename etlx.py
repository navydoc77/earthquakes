...
def extract_transform_load(): 
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS significant_earthquakes (db_id INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, I_D VARCHAR(255), yr INT(5), month INT(3), day INT(3), hr INT(3), minute INT(2), eq_mag_primary DECIMAL(4,2), intensity VARCHAR(255), country VARCHAR(255), location_name VARCHAR(255), lat DECIMAL(10, 6), lng DECIMAL(10,6), deaths INT(10), damage_millions VARCHAR(255), total_deaths INT(10), total_injuries VARCHAR(255), total_damage_millions VARCHAR(255)) ENGINE=InnoDB")
    ...
    #################################################
    sig_earthquakes_df = pd.read_csv('resources/earthquakes_ngdc.csv', header=0)
    # Cleans up the data and omits columns we don't need
#ep    drop_columns = ["I_D", "FLAG_TSUNAMI", "SECOND", "FOCAL_DEPTH", "EQ_MAG_MS", "EQ_MAG_MW", "EQ_MAG_MB", "EQ_MAG_ML", "EQ_MAG_MFA", "EQ_MAG_UNK", "STATE", "MISSING", "MISSING_DESCRIPTION", "INJURIES", "INJURIES_DESCRIPTION", "HOUSES_DESTROYED", "HOUSES_DESTROYED_DESCRIPTION", "HOUSES_DAMAGED", "HOUSES_DAMAGED_DESCRIPTION", "TOTAL_DEATHS_DESCRIPTION", "TOTAL_MISSING", "TOTAL_MISSING_DESCRIPTION", "TOTAL_HOUSES_DESTROYED", "TOTAL_INJURIES_DESCRIPTION", "TOTAL_HOUSES_DESTROYED_DESCRIPTION", "TOTAL_HOUSES_DAMAGED", "DAMAGE_DESCRIPTION", "REGION_CODE", "TOTAL_DAMAGE_DESCRIPTION", "DEATHS_DESCRIPTION", "TOTAL_HOUSES_DAMAGED_DESCRIPTION"]
#ep    sig_earthquakes_df = sig_earthquakes_df.drop(drop_columns, axis = 1)
#ep    sig_earthquakes_df = sig_earthquakes_df.rename(index=str, columns={"YEAR": "yr", "MONTH" : "month", "DAY" : "day", 'HOUR' : "hr", 'MINUTE' : "minute", 'EQ_PRIMARY' : "eq_mag_primary", 'INTENSITY' : "intensity", 'COUNTRY' : "country", 'LOCATION_NAME' : "location_name", 'LATITUDE': "lat", 'LONGITUDE' : "lng", 'DEATHS' : "deaths", 'DAMAGE_MILLIONS_DOLLARS' : "damage_millions", 'TOTAL_DEATHS' : "total_deaths", 'TOTAL_INJURIES' : "total_injuries", 'TOTAL_DAMAGE_MILLIONS_DOLLARS' : "total_damage_millions" })
#ep
#ep    # LOAD SIGNIFICANT EARTHQUAKE DATA INTO TABLE
    sig_earthquakes_df.to_sql('significant_earthquakes', con=engine, if_exists='append', index = False, index_label = "id")
#ep
#ep    # LOAD DATA INTO PANDAS FROM CSV FILES
#ep    df_tornadoes = pd.read_csv('resources/1950-2017_torn.csv')
#ep    df_hail = pd.read_csv('resources/1955-2017_hail.csv')
#ep    df_wind = pd.read_csv('resources/wind.csv')
#ep    df_tsunami = pd.read_csv('resources/tsunami.csv')
#ep    df_volcanoes = pd.read_csv('resources/volcano.csv')

#ep    # LOADING TORNADOES DATA INTO TABLE
#ep    df_tornadoes.to_sql('tornadoes', con=engine, if_exists='append', index = False, index_label = "id")
#ep    # LOADING HAIL DATA INTO TABLE
#ep    df_hail.to_sql('hail', con=engine, if_exists='append', index = False, index_label = "id")
#ep    # LOADING WIND DATA INTO TABLE
#ep    df_wind.to_sql('wind', con=engine, if_exists='append', index = False, index_label = "id")
#ep    # LOADING TSUNAMI DATA INTO TABLE
#ep    df_tsunami.to_sql('tsunamis', con=engine, if_exists='append', index = False, index_label = "tb_id")
#ep    # LOADING VOLCANO DATA INTO TABLE
#ep    df_volcanoes.to_sql('volcanoes', con=engine, if_exists='append', index = False, index_label = "tb_id")
